...
def _test_impl(ctx):
    ...
    if ctx.attr.test_target:
        arguments.add("--run_as_test_target")

    test_target_report = ctx.actions.declare_file("{}_exit_code.sh".format(ctx.label.name))
    outputs.append(test_target_report)
    arguments.add("--test_target_report", "{}".format(test_target_report.path))
